...
func DeployObservability(dryRun bool) error {
	...
	if config.GlobalConfig.Observability.Prometheus {
		...
		fmt.Fprintf(&valuesYAML, "      url: http://cloudinfer-prometheus-kube-prometheus.%s:9090/prometheus\n", ns)
		valuesYAML.WriteString("      alertmanager:\n")
		valuesYAML.WriteString("        enabled: true\n")
		valuesYAML.WriteString("        name: Alertmanager\n")
		valuesYAML.WriteString("        uid: alertmanager\n")
		fmt.Fprintf(&valuesYAML, "        url: http://cloudinfer-prometheus-kube-alertmanager.%s:9093/alertmanager\n", ns)
		...
		if config.GlobalConfig.Observability.Loki {
			...
			fmt.Fprintf(&valuesYAML, "      url: http://cloudinfer-loki.%s:3100\n", ns)
		...
		if config.GlobalConfig.Observability.Tempo {
			...
			valuesYAML.WriteString("      access: proxy\n")
			fmt.Fprintf(&valuesYAML, "      url: http://cloudinfer-tempo.%s:3100\n", ns)
		...
		if config.GlobalConfig.Observability.Pyroscope {
			...
			valuesYAML.WriteString("      access: proxy\n")
			fmt.Fprintf(&valuesYAML, "      url: http://cloudinfer-pyroscope.%s:4040\n", ns)
		...
		if config.GlobalConfig.Observability.BlackboxExporter && len(config.GlobalConfig.Observability.BlackboxTargets) > 0 {
			...
			fmt.Fprintf(&valuesYAML, "            replacement: cloudinfer-blackbox-prometheus-blackbox-exporter.%s:9115\n", ns)
		}
		...
		if config.GlobalConfig.Observability.ArgoCD {
			if err := deployViaArgoCD("cloudinfer-prometheus", "https://prometheus-community.github.io/helm-charts", "kube-prometheus-stack", ns, nil, valuesYAML.String(), dryRun); err != nil {
				return err
			}
		} else {
			tmpValuesPath := filepath.Join(os.TempDir(), "cloudinfer-prometheus-values.yaml")
			...
			if err := helm.InstallRelease("cloudinfer-prometheus", "prometheus-community/kube-prometheus-stack", ns, flags, dryRun); err != nil {
				return err
			}
		}
	...
	if config.GlobalConfig.Observability.Loki {
		...
		if config.GlobalConfig.Observability.ArgoCD {
			if err := deployViaArgoCD("cloudinfer-loki", "https://grafana.github.io/helm-charts", "loki-stack", ns, flags, "", dryRun); err != nil {
				return err
			}
		} else {
			if err := helm.InstallRelease("cloudinfer-loki", "grafana/loki-stack", ns, flags, dryRun); err != nil {
				return err
			}
		}
	}
	...
	if config.GlobalConfig.Observability.Tempo {
		if config.GlobalConfig.Observability.ArgoCD {
			if err := deployViaArgoCD("cloudinfer-tempo", "https://grafana.github.io/helm-charts", "tempo", ns, nil, "", dryRun); err != nil {
				return err
			}
		} else {
			if err := helm.InstallRelease("cloudinfer-tempo", "grafana/tempo", ns, nil, dryRun); err != nil {
				return err
			}
		}
	}
	...
	if config.GlobalConfig.Observability.OpenTelemetry {
		...
		if config.GlobalConfig.Observability.ArgoCD {
			if err := deployViaArgoCD("cloudinfer-otel", "https://open-telemetry.github.io/opentelemetry-helm-charts", "opentelemetry-collector", ns, flags, "", dryRun); err != nil {
				return err
			}
		} else {
			if err := helm.InstallRelease("cloudinfer-otel", "open-telemetry/opentelemetry-collector", ns, flags, dryRun); err != nil {
				return err
			}
		}
	}
	...
	if config.GlobalConfig.Observability.BlackboxExporter {
		if config.GlobalConfig.Observability.ArgoCD {
			if err := deployViaArgoCD("cloudinfer-blackbox", "https://prometheus-community.github.io/helm-charts", "prometheus-blackbox-exporter", ns, nil, "", dryRun); err != nil {
				return err
			}
		} else {
			if err := helm.InstallRelease("cloudinfer-blackbox", "prometheus-community/prometheus-blackbox-exporter", ns, nil, dryRun); err != nil {
				return err
			}
		}
	}
	...
	if config.GlobalConfig.Observability.Pyroscope {
		if config.GlobalConfig.Observability.ArgoCD {
			if err := deployViaArgoCD("cloudinfer-pyroscope", "https://grafana.github.io/helm-charts", "pyroscope", ns, nil, "", dryRun); err != nil {
				return err
			}
		} else {
			if err := helm.InstallRelease("cloudinfer-pyroscope", "grafana/pyroscope", ns, nil, dryRun); err != nil {
				return err
			}
		}
	}
	...
	if config.GlobalConfig.Observability.Thanos {
		...
		thanosFlags := []string{
			"--set", "existingObjstoreSecret=cloudinfer-thanos-objstore",
		}
		if config.GlobalConfig.Observability.ArgoCD {
			if err := deployViaArgoCD("cloudinfer-thanos", "https://charts.bitnami.com/bitnami", "thanos", ns, thanosFlags, "", dryRun); err != nil {
				return err
			}
		} else {
			if err := helm.InstallRelease("cloudinfer-thanos", "bitnami/thanos", ns, thanosFlags, dryRun); err != nil {
				return err
			}
		}
	}
	...
	if config.GlobalConfig.Observability.VictoriaMetrics {
		...
		if config.GlobalConfig.Observability.ArgoCD {
			if err := deployViaArgoCD("cloudinfer-vm", "https://victoriametrics.github.io/helm-charts", "victoria-metrics-k8s-stack", ns, vmFlags, "", dryRun); err != nil {
				return err
			}
		} else {
			if err := helm.InstallRelease("cloudinfer-vm", "vm/victoria-metrics-k8s-stack", ns, vmFlags, dryRun); err != nil {
				return err
			}
		}
	}
	...
	if config.GlobalConfig.Observability.Prometheus {
		...
		fmt.Printf("                               Password command: %s\n", utils.ColorCyan+fmt.Sprintf("kubectl get secret --namespace %s cloudinfer-prometheus-grafana -o jsonpath=\"{.data.admin-password}\" | base64 --decode ; echo", ns)+utils.ColorReset)
		if config.GlobalConfig.Observability.ArgoCD {
...
kind: Secret
metadata:
  name: cloudinfer-thanos-objstore
  namespace: %s
type: Opaque
stringData:
  thanos.yaml: |
...
`, ns)
	...
	tmpPath := filepath.Join(os.TempDir(), "cloudinfer-thanos-objstore.yaml")
...
func applyObservabilityIngress(ns string, dryRun bool) error {
	grafanaSvc := "cloudinfer-prometheus-grafana"
	prometheusSvc := "cloudinfer-prometheus-kube-prometheus"
	alertmanagerSvc := "cloudinfer-prometheus-kube-alertmanager"
	argoSvc := "cloudinfer-argocd-server"
...
func findServiceByPort(ns, nameHint string, port int, fallback string) (string, error) {
	...
	for _, svc := range services.Items {
		...
		if !strings.Contains(name, "cloudinfer") || !strings.Contains(name, nameHint) {
			continue
		}
