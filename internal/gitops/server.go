...
// GitServerManifests returns the deployment and service YAML manifests for stackpulse-git-server.
func GitServerManifests(ns string) string {
	return fmt.Sprintf(`apiVersion: apps/v1
kind: Deployment
metadata:
  name: stackpulse-git-server
  namespace: %s
  labels:
    app: stackpulse-git-server
spec:
  replicas: 1
  selector:
    matchLabels:
...
func DeployGitServer(ns string, dryRun bool) error {
	...
	tmpPath := filepath.Join(os.TempDir(), "stackpulse-git-server.yaml")
	...
	fmt.Printf("%sWaiting for stackpulse-git-server to be ready...\n", utils.PrefixInfo)
	for i := 0; i < 30; i++ {
		_, _, waitErr := utils.ExecCommand("", "kubectl", "wait", "--namespace", ns,
			"--for=condition=Ready", "pod",
			"-l", "app=stackpulse-git-server",
			"--timeout=10s")
		if waitErr == nil {
...
func StartPortForward(ns string, localPort, targetPort int) (func(), error) {
	...
	cmd := exec.CommandContext(ctx, "kubectl", "port-forward", "-n", ns, "svc/stackpulse-git-server", fmt.Sprintf("%d:%d", localPort, targetPort))
...
// IsGitServerRunning checks if the stackpulse-git-server deployment exists in the cluster.
func IsGitServerRunning(ns string) bool {
	out, _, err := utils.ExecCommand("", "kubectl", "get", "deployment", "stackpulse-git-server", "-n", ns, "-o", "jsonpath={.status.readyReplicas}")
	if err != nil {
		return false
	}
